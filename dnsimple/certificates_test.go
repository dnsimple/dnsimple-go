...
func TestCertificatesService_DownloadCertificate(t *testing.T) {
	...
	certificateResponse, err := client.Certificates.DownloadCertificate("1010", "example.com", 2)
	if err != nil {
	...
	if !reflect.DeepEqual(certificate, wantSingle) {
		t.Fatalf("Certificates.DownloadCertificate() returned %+v, want %+v", certificate, wantSingle)
	}
}
...
func TestCertificatesService_GetCertificatePrivateKey(t *testing.T) {
	...
	if !reflect.DeepEqual(certificate, wantSingle) {
		t.Fatalf("Certificates.GetCertificatePrivateKey() returned %+v, want %+v", certificate, wantSingle)
	}
}
